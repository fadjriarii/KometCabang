...
import { GraduateTableRow } from './GraduateTableRow';

export const GraduateTable = ({
  graduates = [],
  data = [],
  totalCount,
  currentPage = 1,
  ...
  const tableList = graduates.length > 0 ? graduates : data;
        ...
        <table className="w-full text-left text-xs min-w-[950px]">
          <thead className="bg-surface-container-low/60 border-b border-surface-container-high text-outline text-[11px] font-semibold uppercase tracking-wider">
            <tr>
              ...
              <th className="py-3 px-4">Nama Wisudawan</th>
              ...
              <th className="py-3 px-4 text-center">Tahun Lulus</th>
          ...
          <tbody className="divide-y divide-surface-container-high/60 bg-surface-container-lowest font-medium">
            {tableList.length === 0 ? (
              <tr>
                <td colSpan={11} className="text-center py-12 text-outline">
                  {loading ? 'Memuat data kelulusan...' : 'Tidak ada data kelulusan yang cocok dengan filter.'}
                </td>
              </tr>
            ) : (
